...
app = Flask('rivus.app', static_url_path='', static_folder=os.path.join(APP_ROOT, "dist"))
...
def configure_logging():
    ...
    class RenameFilter(logging.Filter):
        def filter(self, record):
            if record.name and record.name.startswith("data_formulator"):
                record.name = record.name.replace("data_formulator", "rivus")
            return True
    handler.addFilter(RenameFilter())
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Rivus")
...
def run_app():
    print("Starting Rivus...", flush=True)
